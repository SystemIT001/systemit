...
try {
    ...
    $conn->exec("ALTER TABLE quotes DROP COLUMN payments");
    $conn->exec("ALTER TABLE quotes DROP COLUMN images");
} catch(Exception $e) {
...
elseif ($method === 'POST' || $method === 'PUT') {
    ...
    if ($method === 'PUT') {
        $stmt = $conn->prepare("SELECT lastUpdated FROM quotes WHERE id = :id");
        $stmt->execute([':id' => $id]);
